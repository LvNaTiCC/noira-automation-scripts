...
For controlling edge traffic, DNAT/SNAT is used on hypervisors, forwarding a range of ports (80,443,60000-62000 by default, subject to change) to a virtual router (based on Mikrotik CHR). This configuration is supposed to stay immutable and not a subject to change (as for services as cattle principle).
